...
EventGroupHandle_t wifi_group = NULL;

struct WIFI_STA_DEV wifidev = {
    // .ssid = "Kean",
    // .password = "Kean.2023",
    // .ip = "192.168.8.188",
    // .gateway = "192.168.8.1",
...
void wifi_connect_start(struct WIFI_STA_DEV *WiFi_Config, esp_event_handler_t esp_event_callback)
{
    ...
    inet_aton(wifidev.ip, &ip_inifo.ip);
    inet_aton(wifidev.gateway, &ip_inifo.gw);
    inet_aton(wifidev.subnet, &ip_inifo.netmask);
    ESP_ERROR_CHECK(esp_netif_set_ip_info(esp_netif_get_handle_from_ifkey("WIFI_STA_DEF"), &ip_inifo));
    /**************************************可选 配置静态IP**********************************************/
    /**************************************可选 配置DNS服务器**********************************************/
    esp_netif_dns_info_t dnsInfo = {};
    inet_aton(wifidev.dns, &dnsInfo.ip.u_addr.ip4);
    ...
    */
    ...
    EventBits_t bits = xEventGroupWaitBits(wifi_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT, pdFALSE, pdFALSE, portMAX_DELAY);
    if (bits & WIFI_CONNECTED_BIT)
    {
        ESP_LOGI("TAG", "connected to ap SSID:%s password:%s", wifidev.ssid, wifidev.password);
    }
    else if (bits & WIFI_FAIL_BIT)
    {
        ESP_LOGI("TAG", "Failed to connect to SSID:%s, password:%s", wifidev.ssid, wifidev.password);
    }
}
#else
...
void wifi_connect_start(struct WIFI_STA_DEV *WiFi_Config, esp_event_handler_t esp_event_callback)
{
    ...
    */
    ...
    if (bits & WIFI_CONNECTED_BIT)
    {
        ESP_LOGI(TAG, "已连接WIFI:%s    密码:%s", wifidev.ssid, wifidev.password);
    }
    else if (bits & WIFI_FAIL_BIT)
    {
        ESP_LOGI(TAG, "%s连接失败", wifidev.ssid);
    }
...
void get_sta_wifi_info(void)
{
    ...
    printf("ip: " IPSTR " gateway: " IPSTR "  subnet: " IPSTR "  dns: " IPSTR "\r\n", IP2STR(&ip_info.ip), IP2STR(&ip_info.gw), IP2STR(&ip_info.netmask), IP2STR(&dsn_info.ip.u_addr.ip4));
}