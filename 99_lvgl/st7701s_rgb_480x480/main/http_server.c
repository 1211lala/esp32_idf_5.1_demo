...
esp_err_t http_route_callback(httpd_req_t *req)
{
    ...
    char *buff = NULL;
    printf("开始读取\r\n");
    int len = fatfs_read(path, &buff, 1024 * 10);
    if (len > 0)
    {
...
esp_err_t http_postdata_callback(httpd_req_t *req)
{
    ...
    httpd_query_key_value(rBuf, "ssid", wifidev.ssid, sizeof(wifidev.ssid));
    httpd_query_key_value(rBuf, "password", wifidev.password, sizeof(wifidev.password));

    wifidev.ssid[strlen(wifidev.ssid)] = '\0';
    wifidev.password[strlen(wifidev.password)] = '\0';

    if (wifidev.ssid[0] == '%')
    {
        char zh_ssid[32];
        url_decode(zh_ssid, wifidev.ssid);
        sprintf(wifidev.ssid, "%s", zh_ssid);
        printf("rBuf:%s     ssid:%s     pwd:%s\r\n", rBuf, wifidev.ssid, wifidev.password);
        printf("rBuf:%s     zh_ssid:%s     pwd:%s\r\n", rBuf, zh_ssid, wifidev.password);
    }
    else
    {
        printf("rBuf:%s     ssid:%s     pwd:%s\r\n", rBuf, wifidev.ssid, wifidev.password);
    ...
    strncpy(wifidev.mode, "STA", sizeof(wifidev.mode));
